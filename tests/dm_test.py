...
def test_dm_create_invalid_token(reg_two_users_and_create_dm):
    resp = requests.post(config.url + 'dm/create/v1', json={
                         'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c', 'u_ids': [reg_two_users_and_create_dm['u_id2']]})
...
def test_dm_create_expired_token(reg_two_users_and_create_dm):
    ...
    resp2 = requests.post(config.url + 'dm/create/v1', json={
                          'token': reg_two_users_and_create_dm['token1'], 'u_ids': [reg_two_users_and_create_dm['u_id2']]})
    assert resp2.status_code == A_ERR


def test_dm_create_duplicate_invited_users(reg_two_users_and_create_dm):
    u_ids = [reg_two_users_and_create_dm['u_id2'],
             reg_two_users_and_create_dm['u_id2']]
    resp2 = requests.post(config.url + 'dm/create/v1',
                          json={'token': reg_two_users_and_create_dm['token1'], 'u_ids': u_ids})
...
def test_dm_list_invalid_token(reg_two_users_and_create_dm):
    dmlist = requests.get(config.url + 'dm/list/v1', params={
...
def test_non_creator_remove(reg_two_users_and_create_dm):
    remove = requests.delete(config.url + 'dm/remove/v1', json={
                             'token': reg_two_users_and_create_dm['token2'], 'dm_id': reg_two_users_and_create_dm['dm_id']})
    assert remove.status_code == A_ERR


def test_not_in_dm_remove(reg_two_users_and_create_dm, reg_another_two_users_and_dm):
    remove = requests.delete(config.url + 'dm/remove/v1', json={
                             'token': reg_another_two_users_and_dm['token1'], 'dm_id': reg_two_users_and_create_dm['dm_id']})
    assert remove.status_code == A_ERR


def test_invalid_dm_id(reg_two_users_and_create_dm):
    remove = requests.delete(config.url + 'dm/remove/v1', json={
                             'token': reg_two_users_and_create_dm['token1'], 'dm_id': -9128349012})
...
def test_leave_invalid_token(reg_two_users_and_create_dm):
    leave = requests.post(config.url + 'dm/leave/v1', json={
                          'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c', 'dm_id': reg_two_users_and_create_dm['dm_id']})
    assert leave.status_code == A_ERR


def test_leave_invalid_dm_id(reg_two_users_and_create_dm):
    leave = requests.post(config.url + 'dm/leave/v1', json={
                          'token': reg_two_users_and_create_dm['token2'], 'dm_id': -129831941})
...
def test_dm_create_basic(reg_two_users_and_create_dm):
    resp2 = requests.get(config.url + 'dm/list/v1',
                         params={'token': reg_two_users_and_create_dm['token1']})
...
def test_dm_list_basic(reg_two_users_and_create_dm):
    dmlist = requests.get(config.url + 'dm/list/v1',
                          params={'token': reg_two_users_and_create_dm['token1']})
    assert dmlist.status_code == OK
    dmlist_data = dmlist.json()
    assert dmlist_data['dms'] == [
        {'dm_id': reg_two_users_and_create_dm['dm_id'], 'name': reg_two_users_and_create_dm['dm_name']}]
...
def test_dm_list_multiple(reg_two_users_and_create_dm, reg_another_two_users_and_dm):
    ...
    dmdet1 = requests.get(config.url + 'dm/details/v1', params={
                          'token': reg_two_users_and_create_dm['token1'], 'dm_id': reg_two_users_and_create_dm['dm_id']})
    assert dmdet1.status_code == OK
    dmdet1_data = dmdet1.json()

    dmdet2 = requests.get(config.url + 'dm/details/v1', params={
                          'token': reg_two_users_and_create_dm['token1'], 'dm_id': dmcreate_data['dm_id']})
    assert dmdet2.status_code == OK
    dmdet2_data = dmdet2.json()

    assert dmlist_data['dms'] == [{'dm_id': reg_two_users_and_create_dm['dm_id'], 'name': dmdet1_data['name']}, {
        'dm_id': dmcreate_data['dm_id'], 'name': dmdet2_data['name']}]
...
def test_dm_remove_multiple_people_in_dm(reg_two_users_and_create_dm, reg_another_two_users_and_dm):
    dmcreate = requests.post(config.url + 'dm/create/v1', json={'token': reg_two_users_and_create_dm['token1'], 'u_ids': [
                             reg_another_two_users_and_dm['u_id1'], reg_two_users_and_create_dm['u_id2']]})
    assert dmcreate.status_code == OK
    dmcreate_data = dmcreate.json()
    ...
    dmlist2 = requests.get(config.url + 'dm/list/v1',
                           params={'token': reg_another_two_users_and_dm['token1']})
    assert dmlist2.status_code == OK
    dmlist2_data = dmlist2.json()

    dmlist3 = requests.get(config.url + 'dm/list/v1',
                           params={'token': reg_two_users_and_create_dm['token2']})
...
def test_dm_messages_invalid_token(reg_two_users_and_create_dm):
    resp = requests.get(config.url + 'dm/messages/v1', params={
                        'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c', 'channel_id': reg_two_users_and_create_dm['dm_id'], 'start': 0})
    assert resp.status_code == A_ERR
...
def test_dm_messages_unauthorised(reg_two_users_and_create_dm, reg_another_two_users_and_dm):
    resp = requests.get(config.url + 'dm/messages/v1', params={
                        'token': reg_another_two_users_and_dm['token2'], 'dm_id': reg_two_users_and_create_dm['dm_id'], 'start': 0})
    assert resp.status_code == A_ERR


def test_dm_messages_invalid_channel(reg_two_users_and_create_dm):
    resp = requests.get(config.url + 'dm/messages/v1', params={
                        'token': reg_two_users_and_create_dm['token1'], 'dm_id': -1, 'start': 0})
    assert resp.status_code == I_ERR


def test_dm_messages_big_start(reg_two_users_and_create_dm):
    resp = requests.get(config.url + 'dm/messages/v1', params={
                        'token': reg_two_users_and_create_dm['token1'], 'dm_id': reg_two_users_and_create_dm['dm_id'], 'start': 1})
    assert resp.status_code == I_ERR
